...
async def main():
    ...
    def save_score(player_name, score_value):
        ...
        with open("assets/scores/highscores.txt", "a") as f:
            f.write(f"{player_name},{score_value}\n")

    def load_scores():
        if not os.path.exists("assets/scores/highscores.txt"):
            return []
        with open("assets/scores/highscores.txt", "r") as f:
            lines = f.readlines()
        scores = []
        for line in lines:
    ...
    while running:
        ...
        if doriY >= 650 :
            gameXY(100,250)
            top_scores = load_scores() #top 5 player name showing
            y_offset = 620
